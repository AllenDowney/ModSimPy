...
%
where $C$ is capacitance in farads (\si{\farad}).
...
In the previous chapter we modeled a scenario with constant angular velocity.  In this chapter we make it more complicated, applying constant force to a roll, yielding increasing angular acceleration.
...
Let's get back to the kitten unrolling the toilet paper.  Here's the \py{Params} object with the parameters we'll need:
...
In Chapter~\ref{chap21} we developed a model of air resistance and use a  \py{Params} object, which is a collection of parameters:
