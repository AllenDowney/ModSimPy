...
This function takes the parameters and initial conditions as parameters and returns a new \py{System} object.  The default value for \py{t_end} is 14 weeks, about the length of a semester.


\section{The update function}

At any point in time, the state of the system is represented by a \py{Series} object with three values, \py{s}, \py{i} and \py{r}.  The update function takes this series as a parameter, along with the \py{System} object that contains the parameters, and computes the state of the system during the next time step:
...
The first line uses a feature we have not seen before, {\bf multiple assignment}.  The value on the right side is a \py{Series} that contains three values.  The left hand side is a sequence of three variable names.  The assignment does just what you would want: it assigns the three values from the \py{Series} to the three variables, in order.
...
\py{dtype} indicates the type of the data in the \py{Series}; \py{float64} indicates that it contains 64-bit floating-point numbers.  A {\bf floating-point number} can represent a number with a decimal or fractional part, as opposed to an integer.
...
The previous version of \py{run_model} only returns the state of the system at the end of the simulation, \py{t_end}, but we might want to see how the state changes over time.  We'll consider two ways to do that: first, using three \py{Series} objects, then using one \py{DataFrame}.
...
The first line creates an empty \py{DataFrame} with one column for each of the state variables.

Before the loop starts, we want to store the initial conditions in the \py{DataFrame} at time index 0.  Based on the way we've been using \py{Series} objects, it might be tempting to write:
...
assigns the values from \py{system.init} to the first row of \py{df}.  Since the value on the right-hand side is a \py{Series}, the assignment matches up the index from the \py{Series} with the columns of the \py{DataFrame}; it assigns the \py{S} value from \py{system.init} to the \py{S} column of the \py{DataFrame}, and likewise with \py{I} and \py{R}.

This version of \py{run_model} uses the same feature to write the loop more concisely, writing the results from \py{update_func} directly into \py{df}.  Finally, we store \py{df} as a new attribute of \py{system}.
...
As an exercise, consider writing functions to compute other metrics.
...
Figure~\ref{chap05-fig03} shows the results.  When \py{gamma} is low, the recovery rate is low, which means people are infectious longer.  In that case, even low a contact rate (\py{beta}) results in an epidemic.
...
\begin{figure}
\centerline{\includegraphics[height=3in]{figs/chap05-fig04.pdf}}
\caption{Total number of infected students as a function of the contact number, \py{beta/gamma}.}
\label{chap05-fig04}
\end{figure}

Figure~\ref{chap05-fig04} shows that the results fall neatly on a single curve, at least approximately.  That means that we can predict the fraction of students who will be infected based on a single parameter, the ratio \py{beta/gamma}.  We don't need to know the values of \py{beta} and \py{gamma} separately.
