...
In this chapter, we'll see a better way to find \py{r}, using a SciPy function called \py{root_scalar}.
...
SciPy provides a method called \py{root_scalar} that finds the roots of non-linear equations.  As a simple example, suppose you want to find the roots of the polynomial
%
\[ f(x) = (x - 1)(x - 2)(x - 3) \]
%
...
\index{\py{root_scalar}}
\index{root}

But usually it's not that easy.  In that case \py{root_scalar} can help.  First, we have to write a function that evaluates $f$:
...
Now we call \py{root_scalar} like this:
...
So how can we use \py{root_scalar} to estimate \py{r}?  

What we want is the value of \py{r} that yields a final temperature of \SI{70}{\celsius}.  To work with \py{root_scalar}, we need a function that takes \py{r} as a parameter and returns the difference between the final temperature and the goal:
...
Now we can call \py{root_scalar} like this:
...
\[ C = \rho V c_p \]
%
If the densities of the liquids are equal, they drop out of the equation, and we can write:
%
\[ T = \frac{V_1 T_1 + V_2 T_2}{V_1 + V_2} \]
%
where $V_1$ and $V_2$ are the volumes of the liquids.  

For this problem, we'll assume that milk and coffee have the same density and the same specific heat.
As an exercise, you can look up these quantities to see how good the approximation is.
...
%TODO: Consider adding a figure to show that the two series are visually  identical
...
In Chapter~\ref{chap16} we used \py{root_scalar} to find the value of a parameter that yields a particular result.  We defined an error function:
...
And passed it to \py{root_scalar} with an initial interval, like this:
...
% TODO: Add the pointer to root_scalar

\py{root_scalar} is based on a Scipy function that is also called \py{root_scalar}; you can read more about it at \url{http://modsimpy.com/root_scalar}.
...
The following function uses the \py{Params} object to make a \py{System} object.  This two-step process makes the code more readable and makes it easier to work with functions like \py{root_scalar}.
...
We can find the optimal angle more precisely and more efficiently using \py{maximize_scalar}, like this:

\begin{python}
res = maximize_scalar(range_func, [0, 90], params)
\end{python}

The first parameter is the function we want to maximize.  The second is the range of values we want to search; in this case it's the range of angles from \SI{0}{\degree} to \SI{90}{\degree}.  The third argument can be any object; it gets passed along as an argument when \py{maximize_scalar} calls \py{range_func}.

\index{Params object}

The return value from \py{maximize_scalar} is an \py{ModSimSeries} that contains the results, including \py{x}, which is the angle that yielded the highest range, and \py{fun}, which is the value of \py{range_func} when it's evaluated at \py{x}, that is, range when the baseball is launched at the optimal angle.
...
\item In the previous section the ``optimal" launch angle is the one that maximizes range, but that's not what we want.  Rather, we want the angle that maximizes the height of the ball when it gets to the wall (310 feet from home plate).  So you'll have to write a height function to compute it, and then use \py{maximize_scalar} to find the revised optimum.
...
\item Finally, you'll use \py{root_scalar} to find the velocity that makes the optimal height at the wall just barely 37 feet.
...
Now we can use \py{run_two_phases} to write an error function we can use, with \py{root_scalar}, to find the torque due to friction that yields the observed results from the first push, a total rotation of \SI{1.5}{\radian}.
...
Now we can use \py{root_scalar} to estimate torque due to friction.
...
\begin{python}
res = root_scalar(error_func1, [0.5, 2], params)
force = res.root
\end{python}
...
Then we use \py{maximize_scalar} to find the launch angle that maximizes range:
...
\py{maximize_scalar} uses the SciPy function \py{minimize_scalar}, which uses Brent's method.  You can read about it at \url{http://modsimpy.com/minimize}).

\index{\py{maximize_scalar}}
\index{SciPy}
