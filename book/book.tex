...
\item Maybe the growth rate, \py{alpha}, varies over time.

\item Maybe the net growth from one year to the next depends on the current population, but the relationship is quadratic, not linear.
...
If you compare \py{run_model1} and \py{run_model2}, you will see that they are nearly identical except for the body of the \py{for} loop, where we compute the population for the next year.
...
This function demonstrates a feature we have not seen before: it takes a function as a parameter!  When we run \py{run_any_model}, we provide a state object that contains the parameters of the model and a function, like \py{update_func1} that computes the population for the next year.
...
Now we need two parameters.  I chose these values by trial and error, but we will see better ways to do it later.
...
Figure~\ref{chap03-fig04} shows the result.  The model and the data go together well over the whole range, with just a bit of daylight between them, in the 1960s.

Of course, we should expect the quadratic model to fit better than the linear or proportional model because it has two parameters we can choose to improve the fit, where the other models have only one.  In general, the more parameters you have to play with, the better you should expect the fit to be.

But there is another reason to think the quadratic model might be a good choice: there is a plausible mechanism to explain it.  To understand it, let's look at net growth as a function of population.  Here's how we compute it:
...
Figure~\ref{chap03-fig05} shows the result.  Note that the x axis is not time, as in the previous figures, but population.  
...
At this point we have a model that is simple but plausible, and it fits the data well.  Now we can use it to generate predictions.  In the code, the only thing we have to change is \py{t_end}:
...
\begin{figure}
% chap03.ipynb.py
\centerline{\includegraphics[height=3in]{figs/chap03-fig06.pdf}}
\caption{Quadratic model of world population growth, with projection from 2016 to 2250.}
\label{chap03-fig06}
\end{figure}

Figure~\ref{chap03-fig06} shows the result, with a projection until 2250.  According to this model, population growth will continue almost linearly for the next 50--100 years, then slow over the following 100 years, approaching 13.9 billion by 2250.

I am deliberately using the word ``projection" rather than ``prediction", with the following distinction: ``prediction" implies something like ``this is what we should reasonably expect to happen, at least approximately"; ``projection" implies something like ``if this model is actually a good description of what is happening in this system, and if nothing in the future causes the parameters of the model to change, this is what would happen."

But ``projection" leaves open the possibility that there are important things in the real world that are not captured in the model, or even if the model is good, that the parameters might change.

In this example, the model is based on the assumption that population growth is limited by the availability of resources, so as the population approaches the carrying capacity, birth rates fall and death rates rise because resources become scarce.

If that's assumption is true, it might be possible to use data to estimate the carrying capacity, especially if we have observed the transition into the regime where the growth rate starts to fall.

But in the case of world population growth, none of these conditions apply:
...
\begin{figure}
% chap03.ipynb.py
\centerline{\includegraphics[height=3in]{figs/chap03-fig07.pdf}}
\caption{Projections of world population generated by the U.S. Census Bureau, the United Nations, and our quadratic model.}
\label{chap03-fig07}
\end{figure}

And compare our projections to theirs.  Figure~\ref{chap03-fig07} shows the results.  People who know what they are doing expect world population to grow more slowly than our model projects, probably because their models are broken down by region and country, where conditions are different, and they take into account expected economic development.

Nevertheless, their projects are qualitatively similar to ours, and they differ from each other almost as much as they differ from us.  So the result from this model, simple as it is, are not entirely crazy.
...
\[ x_{n+1} = x_n + c \]
%
where $x_n$ is the population during year $n$, $x_0$ is a given initial population, $c$ is constant annual growth.  This way of representing the model is a {\bf recurrence}; see \url{https://en.wikipedia.org/wiki/Recurrence_relation}.

Often it is possible to solve a recurrence relation, which means to write an equation that computes $x_n$, for a given value of $n$, directly; that is, without computing the intervening values from $x_1$ through $x_{n-1}$.

In the case of constant growth we can see that $x_1 = x_0 + c$, and $x_2 = x_1 + c$.  Combining these, we get $x_2 = x_0 + 2c$, and $x_3 = x_0 + 3c$.  And it is not hard to conclude that in general
%
\[ x_n = x_0 + nc \]
%
...
\[ x_n = x_0 (1 + \alpha)^n \]
%
This result is an example of a {\bf geometric progression}; see \url{https://en.wikipedia.org/wiki/Geometric_progression}.  When $\alpha$ is positive, the factor $1+\alpha$ is greater than 1, so the elements of the sequence grow without bound.
...
\[ x_{n+1} = x_n + c \]
%
We can define to be the change in $x$ from one time step to the next, we can write:
%
...
%
This model is {\bf discrete}, which means it is only defined at the time steps and not in between.  But in reality, people are born and die all the time, not once a year, so a {\bf continuous} model might be more realistic.
...
\[ x(t) = A \exp(\alpha t) \]
%
So there are many solutions to this differential equation, with different values of $A$.  The particular solution we want is the one that has the value $x_0$ when $t=0$.  Well, when $t=0$, $x(t) = A$, so $A = x_0$ and the solution we want is
...
Finally, we can write the quadratic growth model as a differential equation like this:

\[ \frac{dx}{dt} = r x (1 - x/K) \]

If we multiply through by $dt$ and divide through by most of the right-hand side, we get:

\[ \frac{1}{x(1 - x/K)} dx = r dt \]

And again we can take the antiderivative of both sides.

\url{https://www.khanacademy.org/math/ap-calculus-bc/diff-equations-bc/logistic-diff-eq-bc/v/solving-logistic-differential-equation-part-1}


The right-hand side has the form of a line, which is why what we call the constant growth model is more commonly called a ``linear growth model".



The form of this solution is an exponential function, which is why what we have called the proportional growth
...
\item We can use analysis to state and prove generalizations about models; for example, we might certain results will always or never occur.  With simulations, we can show examples and sometimes find counterexamples, but it is hard to write proofs.
...
Each of these languages is generally good for the purposes it was designed for, and less good for other purposes.  But they are often complementary, and one of the goals of this book is to show how they can be used, to best advantage, together.



Exercise: are geometric and exponential the same thing?

Suppose three banks offer the same interest rate, but one compounds monthly, one daily, and one continuously.  What's the difference?

What interest rate, compounded monthly, is equivalent to 3\% compounded continuously?  (Or is there an equivalence?)

What if Polynomial Bank comes along and offers a better return at the end of the first year.  What happens during the second year?
