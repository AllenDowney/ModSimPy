...
Figure~\ref{framework} shows what I mean by ``modeling":

\begin{figure}
\centerline{\includegraphics[height=3in]{figs/modeling_framework.pdf}}
\caption{Framework showing the elements of modeling and simulation.}
\label{framework}
\end{figure}

\begin{itemize}

\item The {\bf system} is something in the real world we are interested in.  Often, it is something complicated, so we have to decide which details can be simplified or {\bf abstracted} away.

\item The result of abstraction is a {\bf model} that includes the features we thinks are essential.  A model can be represented in the form of diagrams and equations, which can be used for mathematical {\bf analysis}.  It can also be implemented in the form of a computer program, which can run {\bf simulations}.

\item The result of analysis and simulation can be a {\bf prediction} about what the system will do, an {\bf explanation} of why it behaves the way it does, or a {\bf design} intended to achieve a purpose.

\item We can {\bf validate} predictions and test designs by taking {\bf measurements} from the real world and comparing the {\bf data} we get with the results from analysis and simulation. 
...
Sometimes a more detailed model is better.  If we include more features and more detail, the model is more realistic, and we expect its predictions to be more accurate.
...
As an example, suppose someone asked you why the orbit of the Earth is nearly elliptical.  If you model the Earth and Sun as point masses (ignoring their actual size) and compute the gravitational force between them using Newton's law of universal gravitation, you can show by analysis or simulation that the orbits in the model are ellipses.

Of course, the actual orbit of Earth is not a perfect ellipse, because of the gravitational forces of the Moon, Jupiter, and other objects in the solar system, and because Newton's law is only approximately true (it does not take into account relativistic effects).
...
So the best modeling decisions depend on what the model is for.  It is usually a good idea to start with a very simple model, even if it is likely to be too simple, and test whether it is good enough for its purpose.  Then you can add features gradually, starting with the ones you expect to be most essential.
...
These essential modeling skills -- abstraction, analysis, simulation, and validation -- are central in engineering, natural sciences, social sciences, medicine, and many other fields.  Some students learn these skills implicitly, but in most programs they are not taught explicitly, and students get little practice.

That's the problem this book is meant to address.  At Olin College, we use this book in a class called Modeling and Simulation, which all students take in their first semester.  My colleagues, John Geddes and Mark Somerville, and I developed this class and taught it for the first time in 2009.

It is based on our belief that modeling can and should be taught explicitly, early, and throughout the curriculum.  It is also based on our conviction that computation is an essential part of this process.
...
And they will only work with bad models; that is, models that are too simple for their purpose.  In nearly every interesting physical system, and especially in engineering, air resistance and friction are essential features; if we ignore them, out predictions will be wrong and our designs won't work.
...
\section{How much programming experience do I need?}
...
\item SymPy for symbolic computation, \url{http://www.sympy.org}.
...
These tools let you work on more interesting programs more quickly, but there are some drawbacks: they can be hard to use, and it can be challenging to keep track of which library does what and how they interact.

I have tried to mitigate these problems by providing a module, called \py{modsim.py} that makes it easier to get started with these tools, and provides some additional capabilities.

Some features in \py{modsim.py} are like training wheels; at some point you will probably stop using them and start working with the underlying libraries directly.  Other parts you might find useful the whole time you are working through the book, or even later.

I encourage you to read the \py{modsim.py} source code.  Most of it is not complicated, and I tried to make it readable.  Particularly if you have some programming experience, you might learn something by reverse-engineering my designs.


\section{How much math and science do I need?} 

I assume that you have studied calculus.  You should know what derivatives and integrals, but that's about all you need to know.  In particular, you don't need to know (or remember) much about finding derivatives or integrals of functions analytically.  If you know the derivative of $x^2$ and you can integrate $x dx$, that will do it.

More importantly, though, you should understand what those concepts {\em mean}.  But if you don't, this book might help you figure it out.

You don't have to know anything about differential equations.

As for science, we will cover topics from a variety of fields, including demography, epidemiology, medicine, thermodynamics, and mechanics. For the most part, I don't assume you know anything about these topics.  In fact, one of the skill you need to do modeling is the ability to learn enough about new fields to develop models and simulations.

But when we get to mechanics, I assume you understand the relationship between position, velocity, and acceleration, and that you are familiar with Newton's laws of motion, especially the second law, which is often expressed as $F = ma$ (force equals mass times acceleration).

I think that's everything you need to know, but if you find that I left something out, please let me know.
...
\item Install Python and the libraries we need on your computer.

\item Copy my files onto your computer.

\item Run Jupyter, which is a tool for running and writing programs, and load a notebook, which is a file that contains code and text.

\end{enumerate}

I wish there were an easier way to get started; it's really unfortunate that you have to do so much work before you write your first program.  But I will try to provide all the information you need.


\section{Installing libraries}

You might already have Python installed on your computer, but you might not have the latest version.  To use the code in this book, you need Python 3.6, or later.  And even if you have the latest version, you probably don't have all of the libraries we need.

You could update Python and install these libraries, but I strongly recommend that you don't go down that road.  I think you will find it easier to use {\bf Anaconda}, which is a free Python distribution that includes all the libraries you'll need (and lots more).
\index{Anaconda}

Anaconda is available for Linux, macOS, and Windows.  By default, it installs all files in your home directory, so you don't need administrator (root) permission to install it, and if you have a version of Python already installed, Anaconda will not remove or modify it.
...
{\bf clone} the repo, which means that you copy the files
to your computer.
\index{fork}

\item Or you can clone my repository directly, without forking.  You don't need a GitHub account to do this, but you won't be able to write your changes back to GitHub.
\index{clone}

\item If you don't want to use Git at all, you can download the files
in a Zip file using the green button in the upper-right corner of the
GitHub page, or this URL: \url{https://github.com/AllenDowney/ModSimPy/archive/master.zip}.  Then you will need a program like WinZip or gzip to unpack the Zip file.

\end{enumerate}

For options 1 and 2, you will need a {\bf git client}, which is a program that manages git repositories.  If you have not used Git before, I recommend GitHub Desktop, which is a simple graphical Git client.  You can download it from \url{https://desktop.github.com/}.

As I write this, GitHub Desktop is not available for Linux.  On Linux, I suggest using the Git command-line client.  Installation instructions are at \url{https://git-scm.com/download/linux}.  Then to clone my repository, you can open a Terminal and run:

\begin{python}
git clone http://github.com/AllenDowney/ModSimPy
\end{python}

Or if you forked my repository on GitHub, you can clone {\em your} repository:

\begin{python}
git clone http://github.com/YourGitHubUserName/ModSimPy
\end{python}
...
As a first example, I'll show you how I computed the results from the previous section using Python.  You can view this example, and the other code in this chapter, at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap01.ipynb}.  At the end of the chapter, I'll give you instructions for downloading and running the examples. 
...
object has, you can print it:

\begin{python}
print(bikeshare)
\end{python}

Or in a Jupyter notebook, just type the name:

\begin{python}
bikeshare
\end{python}

The result looks like a table with the system variable names and their values:
...
that create figures and plot values.
...
To work with this data in Python, we will use functions from the Pandas, which is a library that provides functions for working with data.  It also defines the \py{Series} and \py{DataFrame} object we've been working with.
...
\item There is no obvious mechanism that could cause population growth to be constant from year to year.  Changes in population are determined the fraction of people who die and the fraction of people who give birth, so they should depend on the current population.
...
Let's talk about things that spin.  In general, rotation is complicated:  in three dimensions, objects can rotate around three axes; objects are often easier to spin around some axes than others, and stable when spinning around some axes, but not others.  
...
And define a \py{Condition} object with the parameters of the system:
...
As usual, the slope function takes a \py{State} object, a time, and a \py{System} object.  The \py{State} object contains the hypothetical values of \py{theta}, \py{y}, and \py{r} at time \py{t}.  The job of the slope function is to compute the time derivatives of these values.

The time derivative of \py{theta} is angular velocity, which is often denoted \py{omega}.
...
Now that we've rolled up the paper, let's unroll it.  Specifically, let's simulate a kitten unrolling toilet paper.  As reference material, see this video: \url{https://www.youtube.com/watch?v=1Z9YNmCIscs}.
...
Just as linear acceleration is the derivative of velocity, {\bf angular acceleration} is the derivative of angular velocity.  And just as linear acceleration is caused by force, angular acceleration is caused by the rotational version of force, {\bf torque}.  If you are not familiar with torque, you can read about it at \url{https://en.wikipedia.org/wiki/Torque}.

In general, torque is a vector quantity, defined as the cross product of $\vec{r}$ and $\vec{F}$, where $\vec{r}$ is the {\bf lever arm}, a vector from the point of rotation to the point where the force is applied, and $\vec{F}$ is the vector that represents the magnitude and direction of the force.
...
Since torque is the product of a length and a force, it is expressed in SI units as Newton meters (\si{\newton\meter}).
...
Figure~\ref{paper_roll2} shows the paper roll with $r$, $F$, and $\tau$.  As a vector quantity, the direction of $\tau$ is into the page.  But again, we only care about its magnitude for now.
...
\[ \tau = I \alpha \]
%
Where $\alpha$ is angular acceleration and $I$ is {\bf moment of inertia}.  Just as mass is what makes it harder to accelerate an object\footnote{That might sound like a dumb way to define mass, but its actually one of the fundamental definitions.}, moment of inertia is what makes it harder to spin an object.
...
Fortunately, in a system where all rotation and torque happens around a single axis, we don't have do deal with the most general case.  We can treat moment of inertia as a scalar quantity.
...
At \url{https://en.wikipedia.org/wiki/List_of_moments_of_inertia} you can find moments of inertia for simple geometric shapes.  I'll model the cardboard tube at the center of the roll as a ``thin cylindrical shell", and the paper roll as a ``thick-walled cylindrical tube with open ends".
...
%
where $\rho$ is the density of the material in (mass per volume), $h$ is the height of the tube, $r_2$ is the outer diameter, and $r_1$ is the inner diameter.
...
\item \py{r} is no longer part of the \py{State} object.  Instead, we compute \py{r} at each time step, based on the current value of \py{y}, by Equation~\ref{eqn3} for $r$.

\item Because \py{r} changes over time, we have to compute moment of inertia, \py{I}, at each time step.

\item Angular velocity, \py{omega}, is no longer constant.  Instead, we compute torque, \py{tau}, and angular acceleration, \py{alpha}, at each time step.  \py{odeint} will integrate \py{alpha} to get \py{omega} and integrate \py{omega} to get \py{theta}.

\item I have implicitly changed the definition of \py{theta} so positive values of \py{theta} correspond to clockwise rotation, so $dy/dt = -r \omega$; that is, positive values of \py{omega} yield decreasing values of $y$, the amount of paper still on the roll.

\end{itemize} 

The return value is the sequence \py{omega}, \py{alpha}, and \py{dydt}, which are the derivatives of \py{theta}, \py{omega}, and \py{y}, respectively.

\begin{figure}
\centerline{\includegraphics[height=4in]{figs/chap11-fig02.pdf}}
...
This model yields results that seem reasonable, but remember that I chose the force applied by the kitten arbitrarily, and we ignore friction between the paper roll and the axle.  This example is not meant to be particularly serious, but it is good preparation for the next problem, which is a little more interesting.


\section{Simulating a yo-yo}

In its initial state, suppose a yo-yo has a length of string wound around its axle, and the yo-yo is released while the end of the string is held stationary.  As gravity accelerates the yo-yo downward, tension in the string exerts a force upward.  Since the tension force acts on a point offset from the center of mass, it exerts a torque that causes the yo-yo to spin.
...
In this model, we can't figure out the linear acceleration and angular acceleration independently; we have to solve a system of equations.  Specifically, the equations are $\sum F = m a$ and $\sum \tau = I \alpha$.

As in the previous examples, linear velocity and angular are related because of the way the paper, or string, unrolls:
%
\[ \frac{dy}{dt} = -r \frac{d \theta}{dt} \]
%
...
Taking the derivative of both sides yields a similar relationship between linear acceleration and angular acceleration:
%
...
\[ a = -r \alpha \]
%
But remember that this relationship is not a general law of nature; it is specific to scenarios like this where there is rolling without stretching in the string or tightening in the coil.

Because of the way we've set up the problem, $y$ actually has two meanings: it is the length of the rolled string, but it is also the height of the yo-yo, which decreases as the yo-yo falls.  Similarly, $a$ is both the acceleration in the length of the rolled string and the height of the yo-yo.

And we can compute the acceleration of the yo-yo by adding up the linear forces:

\[ \sum F = T - mg = ma \]

Where tension, $T$ is positive because it points up, and the force of gravity is negative.

Because gravity acts on the center of mass, it creates no torque, so the only torque is due to tension, $T$:

\[ \sum \tau = T r = I \alpha \]

Where positive (upward) tension yields positive (counter-clockwise) angular acceleration.
...
\end{align*}
%
where $I^*$ is the augmented moment of inertia, $I + m r^2$.  To simulate the system, we don't really need $T$, but we can plug $a$ and $\alpha$ directly into the slope function.
