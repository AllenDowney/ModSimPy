...
To find out what directory you are in, type \py{pwd}, which stands for ``print working directory".  On MacOS or Linux, you are probably in your home directory, \py{/home/yourusername}.  One Windows, most likely you are in \py{Users\\yourusername}.

To copy files from my repository to your computer, which is called {\bf cloning} in Git's vocabulary, run this command
...
Whether you clone my repository or yours, you should have a new directory called \py{ModSimPy}.
...
If you downloaded the code in a Zip file, you might have to navigate to whatever directory you downloaded it into.
...
As a first example, I'll show you how I computed the results from the previous section using Python.  You can view this example at \url{http://modsimpy.com/chap01a}.  For instructions on downloading and running the code, see Section~\ref{code}.

% TODO: Fix the links to the notebooks.
...
This example demonstrates analysis and computation using Python.  Next we'll see an example of simulation.  Before you go on, you might want to read the first notebook for this chapter, \py{chap_01_a.ipynb}, and work on the exercises in it.


\section{Modeling a bike share system}
\label{modeling}

You can view the code for the rest of this chapter at \url{http://modsimpy.com/chap01b}.  For instructions on downloading and running the code, see Section~\ref{code}.
...
This example demonstrates the features of Python we'll use to develop computational simulations of real-world systems.  Along the way, I make some decisions about how to model the system.  In the next chapter we'll review these decisions.
...
create a \py{State} object, which is defined in the \py{modsim} library.  
\index{State object}
...
That way, bikes in the model are neither created nor destroyed!
...
When we run the function, we provide a value in parentheses, which gets assigned to the parameter.  So if we call \py{move_bike} like this:

\begin{python}
move_bike(1)
\end{python}

It assigns the value, \py{1}, to the parameter, \py{n}, and then runs the body of the function.  So the effect is the same as:
...
Which moves a bike from Wellesley to Olin.

The value in parentheses is called an {\bf argument}.  If you call \py{move_bike} without providing an argument, you get an error.

Now that we have \py{move_bike}, we can rewrite the other two functions to use it: 
...
At the same time, there might be a different probability that a student at Wellesley wants to ride to Olin:
...
In reality, the probability of an arrival will vary over the course of a day, and might be higher at Olin or Wellesley, at any point in time.  So instead of putting the constant values 0.5 and 0.4 in \py{step} we can replace them with parameters, like this:
\index{probability}
...
The arguments you provide, \py{0.5} and \py{0.4}, get assigned to the parameters, \py{p1} and \py{p2}, in order.  So running this function has the same effect as:
...
\py{TimeSeries} is a specialized version of \py{Series}, which is defined by Pandas.  The \py{Series} object provides many functions; one example is \py{mean}, which we can call like this:
...
Figure~\ref{chap03-fig01} shows the result.

\py{plot} and \py{decorate} library are based on Pyplot, which is a Python for generating figures.  You can read more about \py{plot} and the arguments it takes at \url{http://modsimpy.com/plot}.
...
Before you go on, you might want to read the second notebook for this chapter, \py{chap_01_b.ipynb}, and work on the exercises in it.
...
Here is one of the functions from the previous chapter, \py{move_bike}:
...
When this function is called, it modifies \py{bikeshare}.  As long as there is only one \py{State} object, that's fine, but this function would be more flexible if it took a \py{State} object as a parameter.  Here's what that looks like:
...
Since the new version of \py{move_bike} requires a \py{State} object as a parameter, I have to update the functions that call it:
...
Now I can create as many \py{State} objects as I want:
...
Changes in \py{bikeshare1} do not affect \py{bikeshare2}, and vice versa.  This behavior will be useful later when we create a series of \py{State} objects to simulate different scenarios.
...
If both of these tests pass, we run the last two lines, which assigns the values from the temporary variables to the corresponding state variables.
...
In contrast, the state variables \py{olin} and \py{wellesley} belong to \py{state}, the \py{State} object that was passed to this function as a parameter.  When we change state variables inside a function, those changes are visible in other parts of the program.
...
Inside \py{move_bike}, we update the values of \py{olin_empty} and \py{wellesley_empty}, which are the number of times a customer finds the Olin or Wellesley station empty.

This version of \py{move_bike} will only work if we initialize \py{olin_empty} and \py{wellesley_empty} when we create the \py{State} object, like this:
...
Before you go on, you might want to read the first notebook for this chapter, \py{chap_02_a.ipynb}, and work on the exercises in it.
...
It assigns to \py{state} a new \py{State} object, which we can use to check the metrics we are interested in:
...
\py{range} creates a sequence of numbers from 0 to 3.  Each time through the loop, the next number in the sequence gets assigned to the loop variable, \py{i}.

\index{loop}
\index{loop variable}
\index{variable!loop}

\py{range} only works with integers; to get a sequence of floating-point numbers, we can use \py{linspace}:
...
for p1 in p1_array:
    state = run_simulation(p1, 0.2)
    mark(p1, state.olin_empty, 'rs', label='olin')
\end{python}

When you run the notebook for this chapter, you will see the results and have a chance to try additional experiments.

Before you go on, you might want to read the second notebook for this chapter, \py{chap_02_b.ipynb}, and work on the exercises in it.
...
To fit the model to the data, we'll compute the average annual growth from 1950 to 2015.  Since the UN and Census data are so close, I'll use the Census data.
...
Before you go on, you might want to read the first notebook for this chapter, \py{chap_03_a.ipynb}, and work on the exercises in it.
...
You will not be surprised to learn that the \py{color} argument specifies the color of the line.  For details on color specification in Pyplot, see \url{https://matplotlib.org/users/colors.html}.
...
We will try out these variations, but first, we'll clean up the code one more time.

Before you go on, you might want to read the second notebook for this chapter, \py{chap_03_b.ipynb}, and work on the exercises in it.
...
When you call \py{run_simulation}, it calls \py{update_func1} once for each year between \py{t_0} and \py{t_end}.  The result is the same as Figure~\ref{chap03-fig03}.
...
Before you go on, you might want to read the third notebook for this chapter, \py{chap_03_c.ipynb}, and work on the exercises in it.
...
Before you go on, you might want to read the first notebook for this chapter, \py{chap_04_a.ipynb}, and work on the exercises in it.
...
Before you go on, you might want to read the second notebook for this chapter, \py{chap_04_b.ipynb}, and work on the exercises in it.


\section{DataFrame and Series}
\label{dataframe}
...
We defined a \py{State} object that contains state variables, which describe the state of a system, usually changing over time:
...
In Chapter~\ref{chap02} we used local variables to store intermediate results, comparison operators to check for certain conditions, and the \py{return} statement to end the execution of a function.
...
And we wrote the first version of \py{run_simulation}, which defines the structure of almost every simulation in this book:
...
We used \py{linspace} to create a NumPy array of equally-spaced values, and a \py{for} loop to loop through the array.  We used a \py{SweepSeries} to store results from a series of simulation, mapping from the value of a parameter to the value of a resulting metric.
...
In Chapter~\ref{chap03} we used Pandas to read data from a web page and store the results in a \py{DataFrame}.  We selected a column from the \py{DataFrame} to get a \py{Series}.
...
Chapter~\ref{chap04} introduces SymPy, which we can use to create \py{Symbol} objects:
...
That's a brief summary of the computational tools we have seen so far.  The next two sections present examples where we can use these tools to build models and answer questions about systems.


\section*{One queue or two?}

The first case study asks you to apply what you've learned to a problem from {\bf queueing theory}, which is the study of systems that involve waiting in lines, also known as ``queues".  A solution to this exercise is in \py{queue_soln.ipynb} in the repository for this book.
...
\item Write a version of \py{run_simulation} that uses this update function to simulate the salmon population for 10 years, starting with the 2015 estimate.  It should create and return a \py{TimeSeries} object.

\item Plot the estimated values from 1997 to 2015 and the simulated values from 2015 to 2025.  As always, label the axes!  Each time you run the simulation, you should get different predictions.
