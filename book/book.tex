...
Some features in the \py{modsim} library are like training wheels; at some point you will probably stop using them and start working with the underlying libraries directly.  Other features you might find useful the whole time you are working through the book, and later.
...
Now, the best way to download the files is to use a {\bf Git client}, which is a program that manages git repositories.  You can get installation instructions for Windows, macOS, and Linux at \url{https://git-scm.com/book/en/v2/Getting-Started-Installing-Git}. 
...
If you don't want to use Git, you can download my files
in a Zip archive from \url{http://modsimpy.com/zip}.  You will need a program like WinZip or gzip to unpack the Zip file.  Make a note of the location of the files you download.
...
To start Jupyter on macOS or Linux, open a Terminal; on Windows, open Git Bash.  Use \py{cd} to ``change directory" into the repository:
...
Jupyter should open a window in a browser, and you should see the list of notebooks in my repository.  Click on the first notebook, \py{chap_01_a.ipynb} and follow the instructions to run the first few ``cells".  The first time you run a notebook, it might take a minute or two while some Python files get initialized.  After that, it should run faster.
...
%\part{Modeling a bikeshare system}

\chapter{Falling penny}
\label{chap01}
...
In this chapter we develop a simple model of a bike share system.

demonstrates the features of Python we'll use to develop computational simulations of real-world systems.  
...
As students arrive, check out a bike, and ride to the other campus, the number of bikes in each location changes.  In the simulation, we'll need to keep track of where the bikes are.  To do that, I'll create a \py{State} object, which is defined in the \py{modsim} library.  
...
When you call the function, it runs the statements in the body, which update the variables of the {\tt bikeshare} object; you can check by displaying
...
The first argument of \py{print} is a string, which is a sequence of letters and other symbols in quotes.

The second argument is a variable, \py{n}.  Printing a variable displays its value, which is a number in this example.  Each time you call this version of \py{move_bike}, it displays a message and the value of \py{n}.
\index{string}
...
The result is one of two values: \py{True} with probability 0.7 or \py{False} with probability 0.3.  If you run this function 100 times, you should get \py{True} about 70 times and \py{False} about 30 times.  But the results are random, so they might differ from these expectations.
\index{flip}
\index{True}
\index{False}
...
The arguments, \py{0.5} and \py{0.33}, get assigned to the parameters, \py{p1} and \py{p2}, in order.  So running this function has the same effect as:
...
When we run a simulation, we usually want to save the results for later analysis.  The \py{modsim} library provides a \py{TimeSeries} object for this purpose.  A \py{TimeSeries} contains a sequence of time stamps and a corresponding sequence of values.  In this example, the time stamps are integers representing minutes, and the values are the number of bikes at one location.

%TODO: index modsim library
\index{modsim library}
\index{TimeSeries}
...
%TODO: shorten URL
You can read the documentation of \py{Series} at \url{https://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.html}.
...
The \py{modsim} library provides a function called \py{plot} we can use to plot \py{results}:
...
Whenever you make a figure, you should label the axes.  The \py{modsim} library provides \py{decorate}, which labels the axes and gives the figure a title and legend:
...
Figure~\ref{chap01-fig01} shows the result.
...
%TODO: replace matplotlib with pyplot
...
The first line checks whether the number of bike at Wellesley is zero.  If so, it uses a a {\bf return statement}, which causes the function to end immediately, without running the rest of the statements.  So if there are no bikes at Wellesley, we ``return" from \py{bike_to_olin} without changing the state.
...
It is easy to confuse the equals operators with the assignment operator, \py{=}, which assigns a value to a variable.  For example, the following statement creates a variables, \py{x}, if it doesn't already exist, and gives it the value \py{5}.
...
\py{range} only works with integers; to get a sequence of non-integer values, we can use \py{linspace}:
...
For background on world population growth, watch this video from the American Museum of Natural History \url{https://www.youtube.com/watch?v=PUwmA3Q0_OE}.
...
The next two lines plot the \py{Series} objects.  The {\bf format strings} \py{':'} and \py{'--'} indicate dotted and dashed lines.  For more about format strings in Pyplot, see \url{https://matplotlib.org/api/_as_gen/matplotlib.pyplot.plot.html}.
...
The \py{modsim} library also defines \py{get_first_value} and \py{get_last_value}, which we can use to compute \py{total_growth}:
...
\py{linrange} is defined in the \py{modsim} library.  In this example it returns a NumPy array of integers from \py{t_0} to \py{t_end-1}, including both.
...
For example, in the bike share model, state variables include the number of bikes at each location, which get updated whenever a customer moves a bike.  System variables include the number of location, total number of bikes, and arrival rates at each location.
...
Here's how we create a \py{System} object with these variables:
...
The \py{color} argument specifies the color of the line.  For details on color specification in Pyplot, see \url{https://matplotlib.org/users/colors.html}.
...
Figure~\ref{chap03-fig04} shows the result.  The model fits the data well over the whole range, with just a bit of space between them in the 1960s.
...
Disfunction \#1: Not using parameters.  In the following version, the function doesn't take any parameters; when \py{sys1} appears inside the function, it refers to the object we create outside the function.
...
I avoided these terms until now because they are based on the solutions to problems we had not solved yet.
...
Before you go on, you might want to read the notebook for this chapter, \py{chap09.ipynb}, and work on the exercises.  For instructions on downloading and running the code, see Section~\ref{code}.


\chapter{Case studies}
\label{chap10}

This chapter reviews the computational patterns we have seen so far and presents two exercises where you can apply them.
...
The parameters of \py{run_simulation} are the parameters of the model.  It creates a \py{State} object that represents the initial state of the system, then updates the state over a given number of time steps.  It returns the final value of \py{state}, which contains metrics we can use to measure the performance of the system.
...
%TODO: Update this to reflect the new chapter numbers

We defined another version of \py{run_simulation} with a few changes:

\begin{itemize}

\item As parameters it takes a \py{System} object, which contains the parameters of the system, and an update function, which updates the state of the system.

\item Instead of returning the final state of the system, it returns a \py{TimeSeries} that contains the state as it changes over time.
...
The update functions takes the current state of the system, the time, and the \py{System} object as parameters, and returns the new state.

For example, here's the update function for the quadratic growth model.
...
They type of \py{table2} is \py{DataFrame}.  The type of \py{table2.index} is \py{Int64Index};  The type of \py{table2.columns} is \py{Index}.  \py{Int64Index} and \py{Index} are similar to \py{Series}.
...
The first case study asks you to apply what you've learned to a problem from {\bf queueing theory}, which is the study of systems that involve waiting in lines, also known as ``queues".  

Suppose you are designing the checkout area for a new store.  You have enough room for two checkout counters and a waiting area for customers.  You can make two lines, one for each counter, or one line that feeds both counters.
...
If we choose $\mu=1/5$ per minute, the average time for each checkout will be 5 minutes, which is consistent with the data.
...
\item Write a function called \py{make_system} that takes \py{lam} and \py{mu} as parameters and returns a \py{System} object with variables \py{lam}, \py{mu}, and \py{duration}.  Set duration, which is the number of time steps to simulate, to 10 hours, expressed in minutes.   Test this function by creating a \py{System} object with \py{lam=1/8} and \py{mu=1/5}. 

\item Write a function called \py{run_simulation} that takes a \py{System} object and an update function.  It should simulates the number of time steps specified by \py{duration}.

Use a local variable, \py{x}, to represent the number of customers in the store, including the one checking out.

Start by simulating a single queue with a single checkout counter, which I'll call a ``server".  During each time step, a customer can finish checking out (but only if there is a customer in the store), and a customer can arrive.

\item In \py{run_simulation}, create a \py{TimeSeries} and store the number of customers in the store, \py{x}, at the end of each time step.  After the simulation runs, return the \py{TimeSeries}.  Plot the results and confirm that the simulation behaves as expected.

\item To compute the average number of customers in the store, $L$, you can use the \py{mean} function provided by \py{TimeSeries}.  To compute the average wait time, $W$, you can use Little's Law (see \url{https://en.wikipedia.org/wiki/Little's_law}):
%
\[ L = \lambda W \]
%
The following function takes a \py{TimeSeries} and a \py{System} object, computes $L$ and $W$, and returns them:
...
Test this function using results from your simulations.

\item Use \py{linspace} to create an array of values for $\lambda$, from about 10\% to 90\% of $\mu$.  Write a function called \py{sweep_lam} that takes an array of values for $\lambda$ and a single value for $\mu$.  It should call $run_simulation$ once for each value of $\lambda$; after each simulation, it should print $\lambda$ and the resulting value of $W$.  Encapsulate this code in a function called \py{sweep_lam}.

\item Modify \py{sweep_lam} so it collects the results in a \py{SweepSeries} that maps from each value of \py{lam} to the resulting wait time, \py{W}.  What happens as $\lambda$ gets close to $\mu$?

\item Next, write a simulation that models the scenario with one queue and two servers.  You might be able to do this just by changing the update function.
Hint: if there are at least two customers in the store, there are two chances for a customer to finish checking out.  Test this version with a wider range of values for $\lambda$, maybe going up to 160\% of $\mu$.

\item Write a third version of of the simulation to model the scenario with two queues and two servers.  You might need an additional state variable to keep track of the number of customers in each queue.  When a customer arrives, they should join whichever line is shorter.
...
A solution to this exercise is in \py{queue_soln.ipynb} in the repository for this book.
...
Again, I'll make some suggestions for how to proceed, but if you want to try something different, please do!

\begin{enumerate}

\item In the repository for this book, you'll find a notebook called \py{salmon.ipynb}, which contains some data and code to get you started.  Read through the notebook and run it.

\item Create a \py{System} object with the initial time and population, duration of the simulation in years, and \py{rates}, which is the series of observed annual net growth rates.

\item Write an update function that takes the current population, time, and a \py{System} object, and returns the updated population.  It should choose a random growth rate from \py{rates}.

\item Write a version of \py{run_simulation} that uses this update function to simulate the salmon population for 10 years, starting with the 2016 estimate.  It should create and return a \py{TimeSeries} object.

\item Plot the estimated values from 1997 to 2016 and the simulated values from 2016 to 2025.  As always, label the axes!  Each time you run the simulation, you should get different predictions.

\item To get a sense of how the results vary from one run to the next, write a function that runs multiple simulations and plots all of the results.  If you use the plot option \py{alpha=0.1}, it makes the lines semi-transparent, so they are darker where they overlap.

\item Write another function that runs multiple simulations, extracts the final population from each result, and stores it in a \py{SweepSeries} object called \py{last_pops}.  You can get the mean, median, and other summary statistics from this series by calling
...
\item According to the model, what is the probability that the population will increase over the next 10 years?  

\item How credible do you find these results?  What are the biggest limitations of the model, and what could you do to improve it?

\end{enumerate}

A solution to this exercise is in \py{salmon_soln.ipynb} in the repository for this book.


%\part{Modeling contagious disease}
...
Now let's think about the number of new infections.  Suppose we know that each susceptible person comes into contact with 1 person every 3 days, on average, in such a way that they would transmit the disease if the other person is infected.  We'll denote this contact rate with the Greek letter beta, $\beta$.
...
%TODO: Consider defining `row` as a synonym for `loc`.
...
Before you go on, you might want to read the notebook for this chapter, \py{chap10.ipynb}, and work on the exercises.  For instructions on downloading and running the code, see Section~\ref{code}.
...
In this example we call \py{linrange} with only one argument; it returns a NumPy array with the integers from 0 to \py{max_doses}, including both.
...
def run_simulation(system, update_func):
    ...
    for i in linrange(system.t0, system.t_end-1):
...
\chapter{The coffee cooling problem}
\label{chap15}
...
To help answer this question, I made a trial run with the milk and coffee in separate containers and took some measurements\footnote{This is fiction.  I usually drink tea, and I bike to work.}:
...
Now we write a version of \py{run_simulation} that simulates a series of time steps from \py{t0} to \py{t_end}:
...
One difference is that it uses \py{linrange} with three parameters, which makes an array of timesteps from \py{t_0} to \py{t_end} with space between timesteps \py{dt}.  The argument \py{endpoint=False} tells \py{linspace} not to include \py{t_end}, so the last value in the array is \py{t_end-dt}.
...
This function uses the value of \py{r} from \py{s1} as the value of \py{r} for the mixture.  If \py{s1} represents the coffee, and we are adding the milk to the coffee, this is probably a reasonable choice.  On the other hand, when we increase the amount of liquid in the coffee cup, that might change \py{r}.  So this is an assumption to we might want to revisit.
...
The final temperatures, before mixing, are \SI{70}{\celsius} and \SI{21.8}{\celsius}.  Then we mix them:
...
%
You can see how I got this solution using SymPy in \py{chap14analysis.ipynb} in the repository for this book.  If you would like to see it done by hand, you can watch this video: \url{http://modsimpy.com/khan3}.
...
To get started, we'll assume that the parameters of the model are known.  We'll implement the model and use it to generate time series for \py{G} and \py{X}.  Then we'll see how to find the parameter values that generate the series that best fits the data.
...
If there are no errors, and the results seem reasonable, we are ready to run the simulation.

Here's one more version of \py{run_simulation}.  It is almost the same as in Section~\ref{coffee_impl}, with one change: it passes \py{t} as an argument to \py{update_func}.
...
def run_simulation(system, update_func):
    ...
    frame = TimeFrame(columns=init.index)
    frame.loc[t0] = init
    ts = linrange(t0, t_end, dt, endpoint=False)
...
With the parameters I chose, the model fits the data reasonably well.  But we can do better.
...
Euler's method is good enough for some simple problems, but there are many better ways to solve differential equations, including an entire family of methods called {\bf linear multistep methods} (see \url{http://modsimpy.com/lmm}).
...
\py{scipy.integrate.odeint} is a wrapper for \py{LSODA}, which is from ODEPACK, a venerable collection of ODE solvers written in Fortran (for the history of ODEPACK, see \url{http://modsimpy.com/hindmarsh}).
...
When we used \py{run_simulation}, we created a \py{System} object with variables \py{t0}, \py{t_end}, and \py{dt}.  When we use \py{run_odeint}, we don't need those variables, but we do have to provide \py{ts}, which is an array or \py{Series} that contains the times where we want the solver to evaluate $G$ and $X$.
...
\begin{python}
results = run_odeint(system2, slope_func)
\end{python}

Like \py{run_simulation}, \py{run_odeint} returns \py{TimeFrame} with one row for each time step and one column for each state variable.  In this example, the rows are the values from \py{ts}, and the columns are the state variables, \py{G} and \py{X}.
...
\item When \py{scipy.optimize.leastsq} runs, it calls \py{error_func} many times, each time with a different set of parameters, until it converges on the parameters that minimize MSE.
...
The parameters of \py{error_func} are \py{params}, which is a sequence of four system parameters, and \py{data}, which is the \py{DataFrame} containing the measurements.

\py{error_func} uses \py{make_system} to create a \py{System} object we can use with \py{odeint}:
...
\py{error_func} calls \py{run_odeint} using the same slope function we saw in Section~\ref{slopefunc}.  Then it computes the difference between the simulation results and the data.  Since \py{results.G} and \py{data.glucose} are both \py{Series} objects, the result of subtraction is also a \py{Series}.
...
The parameters of \py{fit_leastsq} are:

\begin{itemize}

\item \py{error_func}, which is the function we just defined.

\item \py{params}, which is a sequence containing an initial guess for the four system parameters.

\item \py{data}, which is the \py{DataFrame} containing the measurements.
\py{fit_leastsq} doesn't do anything with \py{data} other than pass it along to \py{error_func}.

\end{itemize}

\index{\py{fit_leastsq}}

The return value from \py{fit_leastsq} is the sequence of parameters that yields the best fit for the data. 
...
\section{Something thermal?}

\section{The insulin minimal model}

%TODO: transition into this section
...
\py{ts} contains equally spaced points between \SI{0}{\second} and \SI{10}{\second}, including both.
...
Before calling \py{run_odeint}, it is a good idea to test the slope function with the initial conditions:
...
\item[Pandas], which provides the \py{Series} object, which is the basis of the \py{State} object in Section~\ref{modeling}.

\item[Pyplot], which provides plotting functions, as we saw in Section~\ref{plotting}.
