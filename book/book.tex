...
Nevertheless, their projections are qualitatively similar to ours, and theirs differ from each other almost as much as they differ from ours.  So the results from this model, simple as it is, are not entirely unreasonable.
...
This case study is based on ``Height-Age Curves for Planted Stands of Douglas Fir, with Adjustments for Density", a working paper by Flewelling, Collier, Gonyea, Marshall, and Turnblom, available from \url{http://www.cfr.washington.edu/research.smc/working_papers/smc_working_paper_1.pdf}.
...
In the previous chapter we modeled a scenario with constant angular velocity.  In this chapter we make it (slightly) more complex; we'll model a scenario with constant angular acceleration.
...
\texttt{make\_system} creates the initial state, \texttt{init}, and
...
abruptly. We could implement the slope function with an \texttt{if}
statement that checks the value of \texttt{theta} and sets
\texttt{force} accordingly. And for a coarse model like this one, that
...
\item
  ...
  \texttt{theta} is 0.5 radians.
\item
  During the second phase, force is 0, and we run until \texttt{omega}
  is 0.
\end{enumerate}
...
Here's the event function I'll use for Phase 1; it stops the simulation when \texttt{theta} reaches \texttt{theta\_end}, which is when I stop pushing:
...
Pandas provides \texttt{combine\_first}, which combines
\texttt{results1} and \texttt{results2}.
...
But before we call \py{solve}, we have to deal with a problem.  In order to support computation with units, the \py{modsim} library uses a certain amount of black magic.  With most units, the magic works pretty well, but some units are problematic, including Newtons.
...
Now we can use \texttt{fsolve} to estimate torque due to friction.
...
Before we go on, I'll store the result in a global variable.  Because \py{res} is an array, we have to selected the first (and only) element:
...
Here's an error function that takes force and returns the offset from
the remaining angle.  Notice that this function uses the global value of \py{torque_friction}.
...
Now we can run \py{fsolve} again.  This time \py{guess} is a hypothetical value for \py{force}, not \py{torque_friction}:
...
The result is \SI{1.09}{\newton}, so it looks like I just have to push slightly harder than the first push to deliver the teapot
to the desired location.

In the notebook for this chapter, you can run this simulation with different values for the weight and size of the turntable, or weight and location of the teapot.

This problem also lends itself to analysis.  As a challenge, see if you can solve it without doing any simulation.
...
You can see how I got these results in \py{chap25sympy.ipynb}, in the repository for this book.

At this point you have everything you need to simulate the descent of a yo-yo.  In the notebook for this chapter, \py{chap25.ipynb}, you will have a chance to finish off the exercise.  For instructions on downloading and running the code, see Section~\ref{code}.
...
In Chapter~\ref{chap23} we define a range function that computes the distance a baseball flies as a function of launch angle and velocity:
...
Chapter~\ref{chap24} and Chapter~\ref{chap25} introduce the physics of rotation and torque, but no new computational tools.

With that, you have a reasonably complete toolkit for computational modeling and simulation.
...
\py{max_bounded} uses the SciPy function \py{minimize_scalar}, which uses Brent's method.  You can read more about at \url{http://modsimpy.com/minimize}).
...
\item \py{r} is no longer part of the \py{State} object.  Instead, we compute \py{r} at each time step, based on the current value of \py{y}, using Equation~\ref{eqn3}.
...
\item I changed the definition of \py{theta} so positive values of \py{theta} correspond to clockwise rotation, so \py{dydt = -r * omega}; that is, positive values of \py{omega} yield decreasing values of \py{y}, the amount of paper still on the roll.
