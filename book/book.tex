...
Now, the best way to download the files is to use a {\bf Git client}, which is a program that manages git repositories.  You can get installation instructions for Windows, macOS, and Linux at \url{https://modsimpy.com/getgit}. 
...
%TODO: shorten URL
You can read the documentation of \py{Series} at \url{https://modsimpy.com/series}.
...
The model we have so far is simple, but it is based on unrealistic assumptions.  Before you go on, take a minute to review the code from the previous chapter, which you can view at \url{https://modsimpy.com/chap01}.  This code is a representation of the model.  What assumptions is it based on?  Make a list of ways this model might be unrealistic; that is, what are the differences between the model and the real world?
...
For background on world population growth, watch this video from the American Museum of Natural History \url{https://modsimpy.com/human}.
...
The \py{color} argument specifies the color of the line.  For details on color specification in Pyplot, see \url{https://modsimpy.com/color}.
...
This case study is based on ``Height-Age Curves for Planted Stands of Douglas Fir, with Adjustments for Density", a working paper by Flewelling, Collier, Gonyea, Marshall, and Turnblom, available from \url{https://modsimpy.com/trees}.
...
\py{scipy.integrate.solve_ivp} can use one of several ODE solvers; by default it uses a {\bf Runge-Kutta-Fehlberg method}, specifically RKF45 (see \url{https://modsimpy.com/runge}).  This method is {\bf adaptive}, which means it chooses the step size automatically, using small steps when necessary, to limit errors, and large steps when possible, to be efficient.
...
The following circuit diagram\footnote{From \url{https://modsimpy.com/divider}} shows a low-pass filter built with one resistor and one capacitor.  
...
\py{run_ode_solver} uses Brent's method to estimate the time of the event precisely (see \url{https://modsimpy.com/brent}).
...
\item After the cord is fully extended, it obeys Hooke's Law; that is, it applies a force to the jumper proportional to the extension of the cord beyond its resting length.  See \url{https://modsimpy.com/hooke}. 
...
If you enjoy this exercise, you might be interested in this paper: ``How to hit home runs: Optimum baseball bat swing parameters for maximum range trajectories", by Sawicki, Hubbard, and Stronge, at \url{https://modsimpy.com/runs}.
...
At \url{https://modsimpy.com/bungee} you'll find a paper\footnote{Heck, Uylings, and Kędzierska, ``Understanding the physics of bungee jumping", Physics Education, Volume 45, Number 1, 2010.} that explains this phenomenon and derives the acceleration of the jumper, $a$, as a function of position, $y$, and velocity, $v$:
%
...
If you don't believe that their model is correct, this video might convince you: \url{https://modsimpy.com/drop}.
...
\item According to the Spider-Man Wiki\footnote{\url{https://modsimpy.com/spider}}, Spider-Man weighs \SI{76}{\kg}.
