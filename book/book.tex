...
\item I am grateful to Lisa Downey for her thoughtful and careful copy editing.
...
As a first example, I'll show you how I computed the results from the previous section using Python.  You can view this example, and the other code in this chapter, at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap01.ipynb}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
We can read the variables inside a \py{System} object using ``dot notation'', like this:
...
The first line creates a variable named \py{olin_temp} that gets the number of bikes that {\em would} be at Olin if \py{n} bikes were moved.  I added the suffix \py{_temp} to the name to indicate that I am using it as a ``temporary variable".
...
In that case I use a {\bf return statement}, which causes the function to end immediately, without running the rest of the statements.  So if there are not enough bikes at Olin, we ``return" from \py{move_bike} without changing state.  Similarly, if there are not enough bikes at Wellesley, we return without changing state.
...
The result is \py{8}.  As a more useful example, here's a new function that creates a \py{System} object, runs a simulation, and then returns the \py{System} object as a result:
...
You can view the code for this chapter at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap03.ipynb}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
Now we can select a column from the \py{DataFrame} using dot notation, like selecting a system variable from a \py{System} object:
...
The next two lines plot the \py{Series} objects.  The strings \py{':'} and \py{'--'} indicate dotted and dashed lines.  The \py{color} argument can be any of the color strings described at \url{https://matplotlib.org/users/colors.html}.  The \py{label} argument provides the string that appears in the legend.
...
The result from the simulation is a time series, which is a sequence of dates or times and an associated series of values (see \url{https://en.wikipedia.org/wiki/Time_series}).  In this case, we have a sequence of year and an associated sequence of population estimates with associated year.
...
\begin{python}
for year in linrange(1950, 2015):
...
By default, the last value in the array is \py{stop} (at least approximately).  If you provide the keyword argument \py{endpoint=False}, the last value in the array is \py{stop-step}. 

In this example, the result from \py{linrange} is an array of values from \py{1950} to \py{2015} (including both).  

Each time through the loop, the loop variable \py{year} gets the next value from the array.  Inside the loop, we compute the population for each year by adding the population for the previous year and \py{annual_growth}.
...
def run_simulation1(system):
    ...
    for year in linrange(system.t0, system.t_end):
        results[year+1] = results[year] + system.annual_growth
...
def run_simulation2(system):
    ...
    results[system.t0] = system.p0
    for year in linrange(system.t0, system.t_end):
        births = system.birth_rate * results[year]
        deaths = system.death_rate * results[year]
        results[year+1] = results[year] + births - deaths
    system.results = results
\end{python}
...
\begin{python}
def update_func1(pop, year, system):
...
This function demonstrates a feature we have not seen before: it takes a function as a parameter!  When we call \py{run_any_model}, the second parameter is a function, like \py{update_func1}, that computes the population for the next year.
...
\begin{python}
def update_func1b(pop, year, system):
...
\begin{python}
def update_func3(pop, year, system):
    net_growth = system.alpha * pop + system.beta * pop**2
    return pop + net_growth
\end{python}
...
You can view the code for this chapter at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap04.ipynb}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
Table 3 from \url{https://en.wikipedia.org/wiki/World_population_estimates} contains projections from the U.S. Census and the United Nations DESA:
...
Nevertheless, their projections are qualitatively similar to ours, and theirs differ from each other almost as much as they differ from ours.  So the result from this model, simple as it is, are not entirely crazy.
...
The code for this section and the next is in this notebook: \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap04sympy.ipynb}.  
...
As with a \py{DataFrame}, we can use dot notation to select columns from a \py{TimeFrame}.
...
If the \py{Series} is a \py{TimeSeries}, the label you get from \py{idxmax} is a time or date.  You can read more about these functions in the \py{Series} documentation at \url{https://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.html}.
...
You can view the code for this chapter at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap06.ipynb}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
We can encapsulate that code in a function and store the results in a \py{SweepSeries} object:
...
The \py{modsim} library provides a function named \py{interpolate}, which is a wrapper for \py{scipy.interpolate.interp1d}.  It takes any kind of \py{Series} as a parameter, including \py{TimeSeries} and \py{SweepSeries}, and returns a function.  That's right, I said it returns a {\em function}.
...
You can view the code for this chapter at \url{https://github.com/AllenDowney/ModSimPy/blob/master/code/chap09.ipynb}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
Another useful way to visualize the results is animation.  You can create animations using the \py{plot} function from the \py{modsim} library, with the \py{update} argument, which tells plot that the coordinates you provide should update the old coordinates.  Here's an example:
...
To find the time when the ball hits its peak, we use \py{idxmax}, which searches for the largest value in \py{ys} and returns its label (see \url{https://pandas.pydata.org/pandas-docs/stable/generated/pandas.Series.idxmax.html#pandas.Series.idxmax}).
...
% Maybe intro slice indexing earlier.
...
We can find the optimal angle more precisely and more efficiently using \py{max_bounded}, which is a function in the \py{modsim} library that uses \py{scipy.opimize.minimize_scalar} (see \url{https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize_scalar.html}).
