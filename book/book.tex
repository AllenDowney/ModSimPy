...
As a first example, I'll show you how I computed the results from the previous section using Python.  You can view this example at \url{http://modsimpy.com/chap00}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
You can view the code for the rest of this chapter at \url{http://modsimpy.com/chap01}.  For instructions for downloading and running the code, see Section~\ref{code}.
...
Figure~\ref{fig-dataframe} shows elements that make up a \py{DataFrame} and a \py{Series}.  A \py{DataFrame} contains \py{index}, which labels the rows, \py{columns}, which labels the columns, and \py{values}, which is an array.

A \py{Series} contains \py{name}, \py{index}, and \py{values}.
...
\py{get_first_label} and \py{get_last_label} are defined in \py{modsim.py}; as you might have guesses, they select the first and last labels from the index of \py{census}.  We can use these labels to select the first and last values, then compute \py{total_growth} and \py{annual_growth}:
...
The result from the simulation is a time series, which is a sequence of dates or times and an associated sequence of values (see \url{http://modsimpy.com/timeser}).  In this case, we have a sequence of years and an associated sequence of population estimates.
\index{time series}
