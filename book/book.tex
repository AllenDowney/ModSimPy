...
\item \py{header}: Indicates which row of each table should be considered the header, that is, the labels that identify the columns.  In this case it is the first row (numbered 0).
...
The next two lines plot the \py{Series} objects.  The format strings \py{':'} and \py{'--'} indicate dotted and dashed lines.  For more about format strings in Pyplot, see \url{https://matplotlib.org/api/_as_gen/matplotlib.pyplot.plot.html}.
...
To fit the model to the data, we'll compute the average annual growth from 1950 to 2016.  Since the UN and Census data are so close, I'll use the Census data.
...
\py{linrange} is defined in the \py{modsim} library.  In this example it returns a NumPy array of integers from \py{t_0} to \py{t_end-1}, including the first but not the second.
...
This function is similar to previous versions of \py{run_simulation}, with two small differences:

\begin{itemize}

\item It uses \py{linrange} with three parameters to make an array of timesteps from \py{t_0} to \py{t_end} with space between timesteps \py{dt}.

\item It stores the final temperature as a system variable, \py{T_final}.
