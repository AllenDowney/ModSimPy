...
One problem with these function is that they always use \py{bikeshare}, which is a state object.  As long as there is only one state object, that's not a problem, but it is better for if these functions take a state object as a parameter.  Here's what that looks like:
...
The name of the parameter is \py{state}, rather than \py{bikeshare} as a reminder that the value of \py{state}, when these functions run, could be any state object, not just \py{bikeshare}.
...
\item The first line should be a single sentence that describes what the function does.

\item The following lines should explain what each of the parameters means.

\end{itemize}

The docstring should provide the information someone needs to know use {\em use} the function; it should not include details about how the function works -- that's what comments are for.

A comment is a line of code that begins with a hash symbol, \py{#}.  It usually appears inside a function to explain something that would not be obvious to someone reading the program.
...
To paraphrase two Georges, ``All models are wrong, but some models are more wrong than others."  So far we have developed a simple model of a bike share system, and we can use it to simulate the behavior of the system over time.

In order to keep the program simple, I had to make some unrealistic assumptions.  Before you go on, take a minute to review the code.  If you open the notebook for this chapter, all of the functions from the previous chapter are there, with appropriate docstrings.

This code represents a model of the bikeshare system.  What assumptions is this model based on?  Make a list of ways this model might be unrealistic; that is, what are the differences between the model and the real world?
...
Some of these modeling decisions are better than others.  For example, the first assumption might be reasonable if we simulate the system for a relatively short period of time, like one hour.
...
Then it uses the less-than operator, \py{<}, to compare \py{olin} to \py{0}.  The result of this operator is a boolean.  If it's \py{True}, that means \py{olin} is negative, which means that there are not enough bikes.

In that case we use the {\bf return statement}, which causes the function to end immediately, without running the rest of the statements.  So if there are not enough bikes at Olin, we "return" without changing the state.
...
This new version of \py{move_bike} makes sure we never have negative bikes at either station.  But what about \py{bike_to_wellesley} and \py{bike_to_olin}?  Do we have to update them, too?
...
Because these functions use \py{move_bike}, they get take advantage of the new feature automatically.  We only have to make the change in one place.
