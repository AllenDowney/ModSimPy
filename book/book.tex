...
% TODO: Add the pointer to root_bisect

\py{root_bisect} is based on a Scipy function that is also called \py{root_bisect}; you can read more about it at \url{http://modsimpy.com/root_bisect}.
...
Now we can use \py{run_two_phases} to write an error function we can use, with \py{root_bisect}, to find the torque due to friction that yields the observed results from the first push, a total rotation of \SI{1.5}{\radian}.
