...
This function takes as arguments the current year, current population, and a state object, and returns the computed population for the next year.
...
net_growth_array = (state.alpha * pop_array + 
                    state.beta * pop_array**2)
\end{python}
...
Now we can initialize a state object:
...
Every year at Olin College, a new group of about 90 students come to campus for the first time from around the country and the world.  Most of them arrive healthy and happy, but we should expect at least one, by happenstance, to bring with them some kind of infectious disease.  A few weeks later, predictably, some fraction of the incoming class comes down with what we call ``The Freshman Plague".
...
In the basic version of the model, people who have recovered are considered to be immune to reinfection.  That is a reasonable model for some diseases, but not for others, so it should be on the list of assumptions we should consider when we evaluate the model.

Now let's think about how we should expect the number of people in each of these categories to change over time.  Suppose we know that people with the disease are infectious for a period of 4 days on average.  If 100 people are infected at a particular point in time, if we ignore the particular time each one became infected, we would expect about 1 out of 4 to recover on any particular day.

Putting that a different way, if the time between recoveries is 4 days, the recovery rate is about 1 recovery per 4 days, which we'll denote with the parameter $\gamma$ (which is the Greek letter gamma).  If the total number of people in the population is $N$, and the fraction currently infected is $i$, the total number of recoveries we expect per day is $\gamma i N$.

Now let's think about the number of new infections.  Suppose we know that each susceptible person comes into contact with 1 person every three days, on average, in such a way that they would contract the disease if the other person is infected.  We'll denote this contact rate with the parameter $\beta$ (which is the Greek letter beta).

It's probably not reasonable to assume that we know $\beta$ ahead of time, but later in the chapter we'll see how we could estimate this value based on data from previous semesters.
...
This model assumes that the population is closed, that is, that no one arrives or departs.  So the size of the population, $N$, is constant.  If the total of $s$, $i$, and $r$ is initially 1, it will always be 1.

SIR models are examples of {\bf compartment models}, so-called because they divide the world into discrete categories, or compartments, and describe transitions from one compartment to another.  Compartments are also sometimes called {\bf stocks} and transitions between them are called {\bf flows}.
...
Figure~\ref{???} shows the stock and flow diagram for an SIR model.
...
For a given physical system, there are many possible models.  And once we have chosen a model, there are many ways to represent it; for example, as a stock-and-flow diagram, as a set of differential equations, or as a Python program.
...
As a first step, I'll represent the initial state of the system using a Series object.  As we've seen in previous examples, we can start with an empty Series and add elements:
...
Or we can do the same thing more concisely:

\begin{python}
init = Series([90, 1, 0], index=['S', 'I', 'R'])
\end{python}

This series represents the number of people in each compartment.  To convert to fractions, we can divide through by the total:

\begin{python}
init /= np.sum(init)
\end{python}

To get started, we'll assume we know the time time between contacts and recovery times:
...
So we can compute the parameters of the model:
...
Now we need a System object to contain the parameters and initial conditions:
...
This function takes the parameters and initial conditions as parameters and returns a new System object.  The default value for \py{t_end} is 14 weeks, about the length of a semester.


\section{The update function}

At any point in time, the state of the system is represented by a Series object with three values, \py{s}, \py{i} and \py{r}.  The update function takes this series as a parameter, along with the System object that contains the parameters, and computes the state of the system during the next time step:
...
The first line uses a feature we have not seen before, {\bf multiple assignment}.  The value on the right side is a Series that contains three values.  The left hand side is a sequence of three variable names.  The assignment does just what you would want: it assigns the three values from the Series to the three variables, in order.

The update function computes \py{infected} and \py{recovered} as a fraction of the population, then updates \py{s}, \py{i} and \py{r}.  The return value is a Series that contains the updated values and the same index as \py{state}.
...
\py{dtype} indicates the type of the data in the Series; \py{float64} indicates that it contains 64-bit floating-point numbers.  A {\bf floating-point} can represent a number with a decimal or fractional part, as opposed to an integer.
...
The parameters of \py{run_model} are the System object, which contains the model parameters, initial conditions, and values of \py{t0} and \py{t_end}, and the update function.  The outline of this function should look familiar; it is simular to the function we used for the population model in Section~\ref{nowwithstate}.
...
The previous version of \py{run_model} only returns the state of the system at the end of the simulation, \py{t_end}, but we might want to see how the state changes over time.  We'll see two ways to do that: first, using three Series objects, then using one DataFrame.
...
First, we create the Series objects that will contain the results.  I'll use capital letters \py{S}, \py{I} and \py{R} to indicate that each of these objects is a Series that represents the values of \py{s}, \py{i} and \py{r} as they change over time.  A Series of values like this is called a {\bf time series} (see \url{https://en.wikipedia.org/wiki/Time_series}).

The initial value of \py{state} is a copy of \py{system.init}, which is a Series that contains the initial values of \py{s}, \py{i} and \py{r}.  A variable like \py{state}, which represents the condition of the system at a point in time, is called a {\bf state vector} (see \url{https://en.wikipedia.org/wiki/State-space_representation}).  The values in the state vector are called {\bf state variables}.

It might be confusing, at first, that we are using the same kind of object, a Series, to represent both a time series and a state vector.  You might have to think about it, at first, but you'll get used to it.

Inside the loop, we use \py{update_func} to compute the state vector for the next time step, then use multiple assignment to unpack the three elements of the state vector and assign each of them to the corresponding time series.

At the end of the function, we store the Series objects, \py{S}, \py{I} and \py{R}, as a new attribute of the System object, \py{system}.
...
And then use multiple assignment to unpack the results.
...
\py{S}, \py{I} and \py{R} are Series object that represent time series, which we can plot like this:
...
Figure~\ref{chap05-fig01} shows the result.  Notice that it takes about three weeks (21 days) for the outbreak to get going, and about six weeks (42 days) before it peaks.  The fraction of the population that's infected is never very high, but it adds up.  Eventually, almost half the population gets sick. 


\section{Now with a DataFrame}

If the number of state variables is small, like three, keeping track of three time series objects might not be so bad.  But if there are more, a better alternative is to use a DataFrame.  In Section~\ref{worldpopdata} we got a DataFrame as a result from \py{read_html}.  In this section we'll create a DataFrame and use it to store state variables.
...
The first line creates an empty DataFrame with one column for each of the state variables.

Before the loop starts, we want to store the initial conditions in the DataFrame with the time index 0.  Based on the way we've been using Series objects, it might be tempting to write:

\begin{python}
df[0] = system.init
\end{python}

The problem is that when you use the bracket operator with a DataFrame, it selects a column, not a row.  For example, to select the first column, we could write

\begin{python}
df['S']
\end{python}

The result would be a Series.  To read or write a single row, we have use use \py{loc}, which stands for ``location".  So \py{df.loc[0]} reads a row from a DataFrame and 

\begin{python}
df.loc[0] = system.init
\end{python}

assigns the values from \py{system.init} to the first row of \py{df}.  Since the value on the right-hand side is a Series, the assignment matches up the index from the Series with the columns of the DataFrame, assigning the \py{S} value from \py{system.init} to the \py{S} column of the DataFrame, and so on.

We can use the same feature to write the loop more concisely, writing the results from \py{update_func} directly into \py{df}.

Finally, we store \py{df} as a new attribute of \py{system} named \py{results}.

We can call this version of \py{run_model} like this:
...
As we saw in Section~\ref{series}, we can use dot notation to select columns from the DataFrame.


\section{Metrics}

When we plot time series, we get a view of everything that happened when the model ran, but often we want to boil it down to a small set of numbers, often one, that summarizes the outcome.  These summary statistics are called {\bf metrics}.

In this example, we might want to know the time until the peak of the outbreak, the number of people who are sick at the peak, the number of students who will still be sick at the end of the semester, or the total number of students who will be sick at any point.
...
Now we can run the model for any values of the parameters and display the results:
...
We can create a array with a range of values for \py{beta}:
...
Even better, we can encapsulate that code in a function and store the results in a Series:
...
We can call \py{sweep_beta} like this:
...
The first line uses string operations to assemble a label for the plotted line.  If the value of \py{gamma} is \py{0.25}, the value of \py{label} is the string \py{'gamma = 0.25'}.

When the \py{+} operator is applied to strings, it joins them end-to-end, which is called {\bf concatenation}.  The function \py{str} converts any type of object to a String representation.  In this case, \py{gamma} is a floating-point number, so we have to convert it to a string before trying to concatenate it.
...
Figure~\ref{chap05-fig02} shows the results.  Notice that the x-axis is the parameter \py{beta}, not time.  This figure is a parameter sweep, not a time series.
