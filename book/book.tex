...
As a first step, I'll represent the initial state of the system using a \py{StateVector} object.  As we've seen in previous examples, we can start with an empty \py{StateVector} and add elements:
...
At any point in time, the state of the system is represented by a \py{StateVector} object with three values, \py{S}, \py{I} and \py{R}.  The update function takes this series as a parameter, along with the \py{System} object that contains the parameters, and computes the state of the system during the next time step:
...
The first line uses a feature we have not seen before, {\bf multiple assignment}.  The value on the right side is a \py{StateVector} that contains three values.  The left hand side is a sequence of three variable names.  The assignment does just what you would want: it assigns the three values from the \py{StateVector} to the three variables, in order.

The update function computes \py{infected} and \py{recovered} as a fraction of the population, then updates \py{s}, \py{i} and \py{r}.  The return value is a \py{StateVector} that contains the updated values and the same index as \py{state}.
...
The result is the \py{StateVector}
...
\py{dtype} indicates the type of the data in the \py{StateVector}; \py{float64} indicates that it contains 64-bit floating-point numbers.  A {\bf floating-point number} can represent a number with a decimal or fractional part, as opposed to an integer.
...
TODO: Go back and introduce TimeSeries in Chapter 3.  A \py{Series} of values like this is called a {\bf time series} (see \url{https://en.wikipedia.org/wiki/Time_series}).

The initial value of \py{state} is a copy of \py{system.init}, which is the \py{StateVector} that contains the initial values of \py{S}, \py{I} and \py{R}.  

TODO: Go back and introduce StateVector in Chapter 3.  A variable like \py{state}, which represents the condition of the system at a point in time, is called a {\bf state vector} (see \url{https://en.wikipedia.org/wiki/State-space_representation}).  The values in the state vector are called {\bf state variables}.
...
assigns the values from \py{system.init} to the first row of \py{df}.  Since the value on the right-hand side is a \py{StateVector}, the assignment matches up the index from the \py{StateVector} with the columns of the \py{DataFrame}; it assigns the \py{S} value from \py{system.init} to the \py{S} column of the \py{DataFrame}, and likewise with \py{I} and \py{R}.

We can use the same feature to write the loop more concisely, assigning the \py{StateVector} we get from \py{update_func} directly to the next row of \py{df}.  
