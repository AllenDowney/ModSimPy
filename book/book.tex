...
As a first step, I'll represent the initial state of the system using a \py{State} object.  As we've seen in previous examples, we can start with an empty \py{State} and add elements:
...
def update1(system, state):
    ...
    return State([s, i, r], index=state.index)
\end{python}
...
TODO: Go back and introduce State in Chapter 3.  A variable like \py{state}, which represents the condition of the system at a point in time, is called a {\bf state vector} (see \url{https://en.wikipedia.org/wiki/State-space_representation}).  The values in the state vector are called {\bf state variables}.
