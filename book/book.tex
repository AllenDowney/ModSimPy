...
\begin{python}
total_infected = 1 - s_inf_array
frac_infected_analysis = Series(total_infected, index=c_array)
\end{python}

Recall from Section~\ref{series} that a \py{Series} object contains an index, which is \py{c_array} in this example, and a corresponding sequence of values, which is \py{total_infected}.
