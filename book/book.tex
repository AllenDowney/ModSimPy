...
Jupyter should open a window in a browser, and you should see the list of notebooks in my repository.  Click on the first notebook, \py{chap01.ipynb} and follow the instructions to run the first few ``cells".  The first time you run a notebook, it might take a minute or two while some Python files get initialized.  After that, it should run faster.
...
{\tt downey@allendowney.com}.  Or if you are a GitHub user, send me a pull request!
...
\item I am grateful to John Geddes and Mark Somerville for their early collaboration with me to create Modeling and Simulation of the Physical World, the class at Olin College this book is based on.
...
Before you go on, you might want to read the notebook for this chapter, \py{chap01.ipynb}, and work on the exercises.  For instructions on downloading and running the code, see Section~\ref{code}. %FIX: broken reference?
...
The update functions takes the current state of the system, the time, and the \py{System} object as parameters, and returns the new state.  For example, here's the update function for the quadratic growth model.
...
This chapter presents two cases students that let you practice what you have learned so far.  The first case study is related to {\bf queueing theory}, which is the study of systems that involve waiting in lines, also known as ``queues".  
...
Based on data from other stores, you know that it takes 5 minutes for a customer to check out, on average.  But checkout times are variable: most customers take less than 5 minutes, but some take substantially more.  A simple way to model this variability is to assume that when a customer is checking out, they have the same probability of finishing up during each time step.  I'll denote this probability using the Greek letter mu, $\mu$, or the variable name \py{mu}.
...
As usual, the update function takes a \py{State} object, a time, and a \py{System} object as parameters.
The first line \py{update} uses multiple assignment to extract the current values of \py{G} and \py{X}.
The second line uses \py{unpack} so we can read the system variables without using the dot operator.
...
The first return value is a \py{Params} object with the parameters that yields the best fit for the data.  The second return value is a \py{ModSimSeries} object with more information.
...
In Chapter~\ref{chap12} we used the functons \py{max} and \py{idxmax} to compute metrics:
...
As an option, \py{run_ode_solver} can take an {\bf event function}, which can detect an ``event", like the penny hitting the sidewalk, and end the simulation.

Event functions take the same parameters as slope functions, \py{state}, \py{t}, and \py{system}; and they return a value that passes through \py{0} when the event occurs.  Here's an event function that detects the penny hitting the sidewalk:
...
All it does is return the height of the penny, \py{y}, which passes through \py{0} when the penny hits the sidewalk.  We pass the event function to \py{run_ode_solver} like this:
...
\py{run_ode_solver} can use the event function to estimate the time of the event precisely using Brent's method (see \url{https://en.wikipedia.org/wiki/Brents_method}).
...
We can test the slope function like this:

\begin{python}
slope_func(system.init, 0, system)
\end{python}

And then run the simulation like this:
...
And run the simulation like this:
...
In this example I use \py{t_eval} to evaluate the results at equally-spaced time values.  Figure~\ref{chap09-fig03} shows the results.
...
In this case I use the \py{t_eval} option to evaluate the solution at 101 evenly spaced points.

The result is a \py{TimeFrame} object with one column for each of the state variables, \py{x}, \py{y}, \py{vx}, and \py{vy}.
...
To compute the distance the ball travels in the air, we can use an event function to stop the simulation when the \py{y} coordinate passes through 0, and record the final \py{x} coordinate.  Here's the event function:
...
And here's how we run the simulation:

\begin{python}
results, details = run_ode_solver(system, slope_func, events=event_func)
\end{python}

Since we don't know exactly when the simulation will end, we generally don't use the \py{t_eval} option if we're using an event function.  The time steps in these results are not evenly spaced, so they don't look very good when plotted.

But we can get the flight time like this:
...
\py{max_bounded} uses the SciPy function \py{minimize_scalar}, which you can read about at \url{http://modsimpy.com/minimize}).
...
\begin{python}
run_ode_solver(system, slope_func)
\end{python}
...
At this point you have everything you need to simulate the descent of a yo-yo.  In the notebook for this chapter, \py{chap24.ipynb}, you will have a chance to finish off the exercise.  For instructions on downloading and running the code, see Section~\ref{code}.
